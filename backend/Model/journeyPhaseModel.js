...
class JourneyPhaseModel {
  ...
  updateJourneyPhase(data) {
    ...
    // Tratamento de fallback forçado para garantir que ABSOLUTAMENTE NENHUM parâmetro seja undefined
    const length = data.length !== undefined && data.length !== null ? data.length : (data.width || 230);
    const posX = data.posX !== undefined && data.posX !== null ? data.posX : 0;
    const description = data.description !== undefined && data.description !== null ? data.description : " "; 

    if (!journeyphase_id) {
      return Promise.reject(new Error("ID ausente para update"));
    }
